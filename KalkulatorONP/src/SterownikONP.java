...
public class SterownikONP {
	...
	/**
	 * Metoda zamieniajaca wyrazenia na ONP return wyrazenie w ONP
	 * 
	 * @throws Exception
	 */
	public String zamienNaONP(String wyrazenie) throws Exception {
		...
		if ((wyrazenie.charAt(wyrazenie.length() - 1)) != '=')
			...
			throw new Exception("Iloœæ otwartych nawiasów != iloœci nawiasów zamkniêtych");
		} else if (!sprawdzPoprawnosc(wyrazenie)) {
			throw new Exception("Niepoprawne wyra¿enie.");
		} else {
	...
	public double obliczONP(String onp) throws ArithmeticException {
		...
		if (onp != "") {
			...
			for (int i = 0; i < onp.length(); i++) {
				...
				case '=':
					break;
				// jesli na wyjsciu znajduje sie ktorys z operatorow to
				// zdejmujemy ze stosu dwie liczby i wykonujemy na nich
	...
	private boolean sprawdzPoprawnosc(String wyrazenie) {
		String regex = "(\\(-?)*[0-9]{1,}(\\.[0-9]{1,}!\\)*)?!?=";
		//String regex2= "-?(\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*[\\+-x/\\^v%](\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*){1}([\\+-x/\\^v%](\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*)*=";
		String regex3="-?(\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*[\\+-x/\\^v%](\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*([\\+-x/\\^v%](\\(-?)*[0-9]{1,}(\\.[0-9]{1,})?!?\\)*)*=";
		Pattern pattern = Pattern.compile(regex);
		Pattern pattern2 = Pattern.compile(regex3);
		Matcher matcher = pattern.matcher(wyrazenie);
		Matcher matcher2 = pattern2.matcher(wyrazenie);
		if( matcher.matches()||matcher2.matches())
			return true; 
		return false;
	}

	private boolean sprawdzNawiasy(String wyrazenie) {
		int z = 0;
		int o = 0;
		for (int i = 0; i < wyrazenie.length(); i++) {
	...
	private boolean sprawdzElement(String e) {
		String regex = "[0-9]{1,}(\\.[0-9]{1,})?";
		Pattern pattern = Pattern.compile(regex);
